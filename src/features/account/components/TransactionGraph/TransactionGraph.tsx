...
import { CollectorLineDotProps, LineDotProps, TooltipProps } from './types'
...
export const TransactionGraph = ({ isLoading, transactions }: Props) => {
  ...
  const activeLineDot = TRANSACTION_COLLECTOR.getActiveLineDot()

  const lineDotCount = transactions?.reduce((acc, cul) => {
        ...
        >
          {transactions?.map((transaction, index) => (
            <>
              ...
              <XAxis xAxisId={transaction.id} hide={true} />
            </>
          ))}
